...
def main(args):
    ...
    check_convergence(args.round_i,
                      centroids_fn='centroids-{how}-mk{version}-{round_i}.npy'.format(**vars(args)),
                      tmat_fn='tmatfromclus-{how}-mk{version}-{round_i}.mtx'.format(**vars(args)),
                      dir_path='msms')
...
def check_convergence(round_i, centroids_fn, tmat_fn, dir_path):
    ...
    result_obj.centroids_fn = os.path.abspath(os.path.join(dir_path, centroids_fn))
    result_obj.tmat_fn = os.path.abspath(os.path.join(dir_path, tmat_fn))
    result_obj.round_i = round_i
    result_obj.abspath = dir_path
    _, _, _, _, _, errorval = cm.load(result_obj, helper=cm.load_project_eqdistr)
    if errorval < 0.6:
...
def parse():
    ...
    parser = argparse.ArgumentParser(description="Check the convergence of an msm.",
